...
from target_mysql.utils.singer_target_utils import (
    flatten_record,
    flatten_key,
    generate_sqlalchemy_table,
)
from target_mysql.mysql_loader import MySQLLoader
...
class TargetMySQL:
    def __init__(self, config: Dict) -> None:
        # Store the Config so that we can use it to initiate MySQL Loaders
        #  for various tables
        ...
        # The schemas variable is only used for lookups as the MySQLLoader
        ...
        #  MySQL). This is important for performance: we don't want to send
    ...
    def process_line(self, line: str) -> None:
        ...
        if t == "RECORD":
            ...
            # MySQL Target has to map any input to a relational schema,
            ...
            if "properties" not in o["schema"]:
                raise ValidationError(
                    f"Not supported schema by target-mysql:\n {line}\n"
            ...
            # Create a MySQLLoader for that sqlalchemy Table and
            #  run schema_apply() to create the Schema and/or Table if they
            #  are not there.
            loader = MySQLLoader(table=sqlalchemy_table, config=self.config)
    ...
    def flush_records(self, stream: str) -> None:
        ...
        loaders[stream] has an initialized MySQLLoader for the table defined
