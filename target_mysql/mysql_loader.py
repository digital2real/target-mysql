...
# 1. Compare the sqlalchemy Table definition to what is defined in MySQL
# 2. Use the type to manually execute an ALTER TABLE for updating or
#    adding new columns
MAP_SQLALCHEMY_TO_MYSQL_TYPE = {
    "TEXT": "TEXT",
    "STRING": "STRING",
    "BIGINT": "BIGINT",
    "FLOAT": "FLOAT",
...
class MySQLLoader:
    def __init__(self, table: Table, config: Dict) -> None:
        ...
        self.engine = create_engine(f"mysql://{self.dbuser}:{self.dbpassword}@{self.dbhost}/{self.dbname}")
    ...
    def empty_record(self) -> Dict:
        ...
        the table associated with this MySQLLoader instance.
        ...
        flattened before uploaded to MySQL.
    ...
    def schema_update(self, inspector) -> None:
        ...
        for column in self.table.columns:
            # MYSQL does not support updating existing columns so only add new ones
            if column.name not in existing_columns:
                # A new column to be added to the table
                column_type = MAP_SQLALCHEMY_TO_MYSQL_TYPE[f"{column.type}"]
    ...
    def add_column(self, col_name: str, col_data_type: str) -> None:
        """
        Add the requested column to the MySQL Table defined by self.table
        """
    ...
    def load(self, data: List[Dict]) -> None:
        ...
        logging.debug(f"Loading data to MySQL for {self.table.name}")
        if self.table.primary_key:
            # We have to use MySQL's Upsert but the default MySQL for python
