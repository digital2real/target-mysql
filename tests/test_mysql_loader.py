...
def test_table():
    ...
    return Table(
        ...
        Column("id2", String(32), primary_key=True),
        Column("str_attr", String(255)),
...
class TestMySQLLoader:
    def test_connection(self, config, test_table):
        loader = MySQLLoader(table=test_table, config=config)

        with loader.engine.connect() as connection:
            assert connection

            results = connection.execute("select version()").fetchone()
            logging.info(f"Current MYSQL version: {results[0]}")
    ...
    def test_schema_aply(self, config, test_table):
        ...
        # Check that both the schema and the table can be found in MySQL
